...
def run_exit_cycle(db: DB) -> dict[str, int]:
    ...
    out = {
        "entry_sync": sync_pending_entries(db),
    ...
    out["time_exit"] = trigger_time_exit_orders(db, max_hold_min=int(pol.get("time_exit_min", 15)))
    out["exit_sync"] = sync_pending_exits(db)
    return out
